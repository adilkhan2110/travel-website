...
const useJaipurSightseeing = create((set, get) => ({
  items: [],
  totalCount: 0,
  page: 0,
  rowsPerPage: 10,
...
fetchItems: async (page, rowsPerPage, all = false) => {
  ...
  try {
    const response = await fetch(
      all
        ? `/api/jaipur-sightseeing?all=true`
        : `/api/jaipur-sightseeing?page=${page}&limit=${rowsPerPage}`
    );
...
getItemById: async (id) => {
  ...
  try {
    const response = await fetch(`/api/jaipur-sightseeing/${id}`);
...
addItem: async (formData) => {
  ...
  try {
    const response = await fetch("/api/jaipur-sightseeing", {
      method: "POST",
      body: formData,
    });
...
export default useJaipurSightseeing;
